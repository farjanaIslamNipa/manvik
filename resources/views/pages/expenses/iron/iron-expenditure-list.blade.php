<x-admin-layout>
  ...
  <div class="page-wrapper">
      <div class="container-fluid">
        <div class="row">
          <div class="col-6">
            <h4 class="page-title text-xl font-bold">Laundry Bill</h4>
          </div>
          <div class="col-6">
            <div class="text-end">
              <a class="btn bg-sky-blue text-white me-2" href="{{ route('admin.laundry.bill.add') }}">Add New</span></a>
            </div>
          </div>
        </div>
        ...
        <div class="table-responsive-xxl mt-3">
            <table style=" overflow-x:auto; white-space: nowrap;" class="table table-dark table-hover">
              <thead>
                ...
                <tbody style="">
                    @if(count($laundryBills) > 0)
                    @foreach ($laundryBills as $laundryBill)
                    <tr>
                      <td scope="row">{{ $loop->index + 1 }}</td>
                      <td class="text-capitalize">{{ $laundryBill->shop_details }}</td>
                      <td class="text-capitalize">{{ $laundryBill->prod_name }}</td>
                      <td class="text-center">{{ $laundryBill->quantity }}</td>
                      <td class="text-center">{{ $laundryBill->unit_price }}</td>
                      <td class="text-center">{{ $laundryBill->total_price }}</td>
                      <td class="text-center">{{ $laundryBill->due }}</td>
                      <td class="text-center">{{ $laundryBill->paid }}</td>
                      <td>{{ $laundryBill->date }}</td>
                      <td>{{ $laundryBill->note }}</td>
                      <td>
                        <a class="btn btn-sm btn-success rounded me-1" href="{{ route('admin.laundry.bill.edit', $laundryBill->id) }}"><span><i class="fa-solid fa-pen-to-square"></i></span></a>
                        <button class="btn btn-sm btn-danger rounded delete-laundry-bill"
                            data-bs-toggle="modal"
                            data-bs-target="#deleteLaundryBill"
                            data-id="{{ $laundryBill->id  }}">
    ...
    {{-- delete modal --}}
    <div class="modal fade" id="deleteLaundryBill" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <a href="" type="submit" id="deleteLaundryBillModalHref" class="btn btn-danger bg-danger">Delete it.</a>
                </div>
            </div>
        </div>
    </div>
  ...
  @section('scripts')
  <script>
        $('.delete-laundry-bill').click(function () {

            let id_value = $(this).data('id');
            let url = '/admin/laundry-bill/delete/' + id_value ;
            console.log(url)
            document.getElementById("deleteLaundryBillModalHref").href = url;
        });
